...
void CWalletTx::GetAmounts(int64& nGeneratedImmature, int64& nGeneratedMature, list<pair<CTxDestination, int64> >& listReceived,
                           list<pair<CTxDestination, int64> >& listSent, int64& nFee, string& strSentAcrapunt) const
{
    nGeneratedImmature = nGeneratedMature = nFee = 0;
    listReceived.clear();
    listSent.clear();
    strSentAcrapunt = strFromAcrapunt;
...
void CWalletTx::GetAcrapuntAmounts(const string& strAcrapunt, int64& nGenerated, int64& nReceived,
                                  int64& nSent, int64& nFee) const
{
    ...
    GetAmounts(allGeneratedImmature, allGeneratedMature, listReceived, listSent, allFee, strSentAcrapunt);

    if (strAcrapunt == "")
        nGenerated = allGeneratedMature;
    if (strAcrapunt == strSentAcrapunt)
    {
    ...
    }
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(CTxDestination,int64)& r, listReceived)
        {
            if (pwallet->mapAddressBook.count(r.first))
            {
                ...
                if (mi != pwallet->mapAddressBook.end() && (*mi).second == strAcrapunt)
                    nReceived += r.second;
            }
            else if (strAcrapunt.empty())
            {
                nReceived += r.second;
            }
        }
