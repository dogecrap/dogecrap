...
static const char *strDNSSeed[][2] = {
    {"No host come on don't use this shit", "nothing.no.pla.ce"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. dogecrap is probably already running."), addrBind.ToString().c_str());
        else
